...
const Topic = ({info, order, ancestor, onDeleteReply}: TopicProp) => {
  ...
  const openHideConfirmModal = async () => {
    ...
    try {
      if (topic.comment_id) {
        // await updateOneCommentHide(offerId, topic.comment_id, reqBody);
      } else {
        // await updateOnePostHide(offerId, reqBody);
      }
      // Refresh
  ...
  const openCloseConfirmModal = async () => {
    ...
    // await updateOnePostClose(offerId, reqBody);
    // Refresh
  ...
  const onLikeClick = async () => {
    ...
    const reqBody = {post_id: offerId, user_id: user._id, comment_id: topic.comment_id};
    const {data} = {data: 4};
  ...
  const copyToClipboard = (level: string) => {
    const targetAchorUrl = `${window.location.origin}/#/offer/${offerId}#${level}`;
      ...
      <div className="body">
        <div className="heading">
          <div className="meta">
            ...
            <div className="auth">
              ...
              <div className="datetime">
                ...
                {topic.edit_time && topic.create_time !== topic.edit_time ? (
                  <span>
                    {'  ('}
          ...
          <div className="corner">
            ...
            {!popperMenuOptions.every(({hidden}) => hidden) ? (
              <PopperMenu options={popperMenuOptions}></PopperMenu>
            ) : null}
          </div>
        </div>
        {!order || topic.access === CommentAccess.Public ? (
                    ...
                    <>
                      <Row>
                        ...
                        <ul>
                          <li>
                            <span>Salary: </span>
                            <span>{topic.salary}</span>
                          </li>
                          <li>
                            <span>Bonus: </span>
                            <span>{topic.bonus}</span>
                          </li>
                        </ul>
                      </Row>
                      <Row>
