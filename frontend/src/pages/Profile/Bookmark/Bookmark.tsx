...
const convertToTable = (responseBody): Post[] => {
  return responseBody.map(({post_id, title, category, likes, tags, is_pinned, author, create_time, replies, views}) => {
    return {
      id: post_id,
...
const Bookmark = (prop) => {
  ...
  const queryParams = useRef({skip: 0, limit: 10});
  const getPosts = async () => {
    const resp = await listAllBookmarks(queryParams.current);
  ...
  const options = {
    selectableRowsHeader: get(authState, 'user.role') === 'ADMIN' && false,
    selectableRowsHideCheckboxes: get(authState, 'user.role') !== 'ADMIN' && false,
    print: false,
    pagination: false,
    download: false,
  ...
  return (
    <Container className="Bookmark block">
...
export default withStyles(styles as {})(WithDialog(Bookmark));
