...
const NotificationPanel = ({unread, onRefresh}: {unread: number; onRefresh: (refresh: boolean) => void}) => {
  ...
  const convertNotiDisapy = (notis) => {
    ...
    const interpolateWording = (string, values) =>
      string.replace(curlyBraceRxp, (match, offset) => get(values, offset));
  ...
  const onNotiItemClick = (
    notificationId: string,
    isRead: boolean,
    linkParam: {post_id?: string; comment_id?: string}
  ) => {
    if (!isRead) {
      // (async()=>{
  ...
  const fetchNotifications = async (isReset?: boolean) => {
    ...
    const concatedList = [...prevNotiList, ...convertNotiDisapy({})];
  ...
  return (
    <Box className="NotificationPanel">
      ...
      <Popper
        ...
        placement="bottom"
        transition
        sx={{width: '20rem', maxHeight: '50vh', zIndex: (theme) => theme.zIndex.drawer + 1}}
      >
        {({TransitionProps}) => (
              ...
              {notiList.length > 0 ? (
                <List
                  ...
                  subheader={
                    <ListSubheader
                      className="header"
                      sx={{display: 'flex', justifyContent: 'space-between', top: 'inherit'}}
                    >
                      <div className="title">Notifications</div>
                      <div className="clear">
                        <Link href="#" onClick={onMarkAllRead}>
                          Mark all read
                        </Link>
                      </div>
                    </ListSubheader>
                  ...
                  >
                    {notiList.map((noti, index) => {
                      return (
                        <ListItemButton
                          key={'noti_' + index}
                          onClick={() => onNotiItemClick(noti.notification_id, noti.is_read, noti.link)}
                        >
                          <ListItemAvatar>
                            <Avatar alt={noti.sender.username} src={noti.sender.avatar} />
                          </ListItemAvatar>
                          <ListItemText
                            primary={noti.context}
                            secondary={TimeFormatter.timeSince(new Date(noti.create_time))}
                          />
                          {!noti.is_read && (
                            <Brightness1RoundedIcon color="success" sx={{fontSize: 10}} fontSize="small" />
                          )}
                        </ListItemButton>
                      );
                    })}
                ...
                <Typography sx={{p: 2}}>There is no notification.</Typography>
              )}
            </Paper>
          </Fade>
        )}
