...
const imageDecorator = composeDecorators(focusPlugin.decorator, resizeablePlugin.decorator);
...
const CustomToolbarEditor: React.FC<EditorProps<string>> = ({
  onChange,
  formControlName,
  value,
  name,
  ...
  const insertImage = (src: string) => {
    ...
    const contentStateWithEntity = contentState.createEntity('IMAGE', 'IMMUTABLE', {src, height: 100, width: 100});
  ...
  return (
    <Box className="field" onClick={focus}>
      <InputLabel sx={{color: '#212529'}} className="label">
        {name}
      ...
      <Box className="editor" sx={{marginLeft: 0, marginRight: 0}}>
            ...
            (externalProps) => (
              <div>
                ...
                {toolbar.image && (
                  <ImageButton
                    {...externalProps}
                    option={{...get(toolbar, 'image'), insertImageCallback: insertImage}}
                  />
                )}
              </div>
            )
        ...
        <Editor editorState={editorState} onChange={onEditorChange} plugins={plugins} ref={editorRef} />
        <AlignmentTool />
      </Box>
      {/* <textarea
          disabled
