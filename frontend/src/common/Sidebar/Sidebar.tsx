...
const CATEGORIES = [
  {label: 'News', value: null, default: true},
...
const Sidebar = () => {
  ...
  const [selected, setSelected] = useState<{[group: string]: Set<string | boolean>}>({
    category: new Set<string>(),
    tags: new Set<string>(),
    hidden: new Set<boolean>()
  });
        ...
        <div className="group">
          ...
          <ul className="categories">
            {CATEGORIES.map(({label, value}, key) => {
              return (
                <li
                  className={value && selected.category.has(value) ? 'selected' : ''}
                  key={key}
                  onClick={() => onFilterChipClicked('category', value, true)}
                >
                  <div>{label}</div>
        ...
        <div className="group">
          ...
          <ul className="tags">
            {TAGS.map(({label, value}, key) => {
              return (
                <li
                  className={value && selected.tags.has(value) ? 'selected' : ''}
                  key={key}
                  onClick={() => onFilterChipClicked('tags', value)}
                >
                  <div>{label}</div>
          ...
          <div className="group">
            ...
            <ul className="display">
              {DISPLAY.map(({label, value}, key) => {
                return (
                  <li
                    className={value !== null && selected.hidden.has(value) ? 'selected' : ''}
                    key={key}
                    onClick={() => onFilterChipClicked('hidden', value !== null ? value : null, true)}
                  >
                    <div>{label}</div>
